...
export const activateUser = async (req: IActivationRequest, res: Response) => {
...
export const registerUser = (req: Request, res: Response) => {
  ...
  User.findOne({ email }).then(user => {
    if (user) {
      ...
      // TODO: provide salt for bcrypt
      const newUser = new User({
        firstName,
        lastName,
        email,
        password: bcrypt.hashSync(password, "salt"),
        activationKey: bcrypt.hashSync(email, "salt").replace(/\//g, "")
      });
  ...
  if (user) {
    ...
    if (email) {
      ...
      if (email !== user.email) {
        ...
        user.email = email;
        // TODO provide salt
        user.activationKey = bcrypt.hashSync(email, "salt").replace(/\//g, "");
